...
public class CreateCourseTest extends BaseClass {
	@Test
	public void creatCourseTest() throws InterruptedException {
		SoftAssert soft = new SoftAssert();
		home.clickcourseTab();
		home.clickCourseListLink();
		soft.assertTrue(course.getPageHeader().contains("Course"));
		...
		addCourse.uoploadphoto(map.get("Photo"));
		
		addCourse.setDescription(webUtil, map.get("Description"));
		addCourse.clickSaveutton();
		soft.assertTrue(course.getSuccessMessage().contains("Success"));
		boolean isPresent=false;
		List<WebElement> coueseNameList=course.getCourseList();
		for(WebElement e: coueseNameList) 
		{
